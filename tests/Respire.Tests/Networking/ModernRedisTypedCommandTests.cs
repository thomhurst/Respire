...
public class ModernRedisTypedCommandTests
{
    ...
    public async Task StringModernCommands_WriteExpectedFramesAndParseReplies()
    {
        ...
        var setExpireAt = await client.Strings.SetManyExpireAtAsync(
            DateTimeOffset.FromUnixTimeMilliseconds(1111111111), ("c", "3"));
        var keepExpiry = await client.Strings.SetManyKeepExpiryAsync(SetWhen.NotExists, ("d", "4"));
    ...
    public async Task ModernCommands_RejectInvalidShapesBeforeSending()
    {
        ...
        await Assert.That(async () => await client.Strings.SetManyExpireAsync(TimeSpan.FromSeconds(1)))
            .Throws<ArgumentException>();
        await Assert.That(async () => await client.Strings.SetManyKeepExpiryAsync((SetWhen)42, ("key", "value")))
            .Throws<ArgumentOutOfRangeException>();
