...
/// <summary>
/// MSETEX behaviour against a real server: the collapsed <c>SetManyAsync(RespireTtl, SetWhen, …)</c>
...
public class SetManyExpiryIntegrationTests(ModernRedisTestContainer fixture)
{
    ...
    public async Task SetMany_WithAbsoluteTtl_EndsAtThatInstant()
    {
        ...
        (await client.Strings.SetManyAsync(RespireTtl.At(instant), pairs: ("msetexat:a", "1"))).Should().BeTrue();
    ...
    public async Task SetMany_WithKeep_RetainsTheExistingTtl()
    {
        ...
        (await client.Strings.SetManyAsync(RespireTtl.Keep, pairs: ("msetkeep:a", "second"))).Should().BeTrue();
    ...
    public async Task SetMany_WithConditions_GatesTheWrite()
    {
        ...
        (await client.Strings.SetManyAsync(
            RespireTtl.Keep, SetWhen.Exists, ("msetnx:a", "third"))).Should().BeTrue();
        (await client.GetStringAsync("msetnx:a")).Should().Be("third");
        (await client.Keys.ExpiryAsync("msetnx:a")).HasExpiry.Should().BeTrue();

        (await client.Strings.SetManyAsync(
            RespireTtl.Keep, SetWhen.Exists, ("msetnx:missing", "value"))).Should().BeFalse();
