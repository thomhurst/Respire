...
public class ClientComparisonTests
{
    ...
    public async Task StringOperations_ShouldProduceSameResults()
    {
        ...
        var respireGet = await _respireClient.GetStringAsync(key);

        // Assert
        respireGet.Should().Be(value);
    ...
    public async Task IncrementOperations_ShouldProduceSameResults()
    {
        ...
        var r1 = await _respireClient.IncrementAsync(key);   // 2
        var se2 = await _stackExchangeDb.StringIncrementAsync(key);  // 3
        var r2 = await _respireClient.IncrementAsync(key);    // 4
        ...
        var finalValueR = await _respireClient.GetStringAsync(key);

        finalValueSE.ToString().Should().Be("4");
        finalValueR.Should().Be("4");
    }
    ...
    {
        // Act
        var respireResult = await _respireClient.PingAsync();
        var stackExchangeResult = await _stackExchangeDb.PingAsync();

        // Assert - Just verify they both succeed
        respireResult.Should().BeGreaterThan(TimeSpan.Zero);
    ...
    public async Task MixedOperations_ShouldMaintainConsistency()
    {
        ...
        await _respireClient.DeleteAsync(listKey);
    ...
    public async Task ExpireAndTTL_Operations_ShouldWork()
    {
        ...
        await _respireClient.ExpireAsync(key, TimeSpan.FromSeconds(expireSeconds));
    ...
    public async Task VariousKeyFormats_ShouldWork(string keySuffix)
    {
        ...
        // Get with Respire
        var retrievedR = await _respireClient.GetStringAsync(key);
        retrievedR.Should().Be(newValue);
    }
    ...
    public async Task NullAndEmptyValues_ShouldBeHandledConsistently()
    {
        ...
        var emptyFromRespire = await _respireClient.GetStringAsync(emptyKey);
        emptyFromRespire.Should().Be("");

        // Try to get non-existent key
        var nullFromSE = await _stackExchangeDb.StringGetAsync(nullKey);
        var nullFromRespire = await _respireClient.GetStringAsync(nullKey);

        nullFromSE.IsNull.Should().BeTrue();
        nullFromRespire.Should().BeNull();
    }
}
