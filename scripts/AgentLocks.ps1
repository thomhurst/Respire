...
# resolves from -OwnerId, RESPIRE_AGENT_LOCK_OWNER_ID, or CODEX_THREAD_ID (in that order).
...
#   $env:RESPIRE_AGENT_LOCK_OWNER_ID = 'my-agent-run-id'
...
$stateRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'respire-agent-locks'
...
function Resolve-OwnerIdentity([string]$Explicit) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:RESPIRE_AGENT_LOCK_OWNER_ID)) {
        return $env:RESPIRE_AGENT_LOCK_OWNER_ID.Trim()
    }
...
if (-not $OwnerId) {
    Die 2 'cannot resolve lock owner identity: pass -OwnerId or set RESPIRE_AGENT_LOCK_OWNER_ID (CODEX_THREAD_ID is used automatically in Codex)'
}
...
$lockKey = "respire:agent-lock:$LockName"
$metaKey = "respire:agent-lock-meta:$LockName"
