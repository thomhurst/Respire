...
$redisContainer = 'respire-agent-locks-redis'
$suffix = [Guid]::NewGuid().ToString('N')
$issueNumber = Get-Random -Minimum 100000 -Maximum 999999
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) "respire-agent-lock-owner-tests-$suffix"
...
function Invoke-AgentLocks {
    ...
    $previousOwnerId = [Environment]::GetEnvironmentVariable('RESPIRE_AGENT_LOCK_OWNER_ID', 'Process')
    $previousThreadId = [Environment]::GetEnvironmentVariable('CODEX_THREAD_ID', 'Process')

    try {
        [Environment]::SetEnvironmentVariable('RESPIRE_AGENT_LOCK_OWNER_ID', $EnvironmentOwnerId, 'Process')
        ...
        [Environment]::SetEnvironmentVariable('RESPIRE_AGENT_LOCK_OWNER_ID', $previousOwnerId, 'Process')
...
function Expire-TestLock([string]$LockName) {
    $deleted = & docker exec $redisContainer redis-cli DEL "respire:agent-lock:$LockName" "respire:agent-lock-meta:$LockName"
...
function Assert-OwnerIdentityNotExposed([string]$LockName, [string]$OwnerId) {
    $meta = & docker exec $redisContainer redis-cli GET "respire:agent-lock-meta:$LockName"
...
function Remove-TestLocks([string[]]$LockNames) {
    $keys = @($LockNames | ForEach-Object { "respire:agent-lock:$_"; "respire:agent-lock-meta:$_" })
