...
$worktreeRoot = Join-Path (Split-Path $repo -Parent) "Respire-worktrees"
...
If the sandbox cannot write to the sibling directory, use `C:\tmp\Respire-worktrees` instead.
...
Respire has no Aspire AppHost. Do not run `aspire start`, `aspire stop`, or AppHost cleanup commands in this repo.

Integration tests use Testcontainers to spin up ephemeral Redis containers (`tests/RedisTestContainer.cs`); Testcontainers manages their lifecycle. Only stop processes or containers you started for the current worktree. Never blanket-delete Docker containers or volumes; another agent or local test run may be using them (the `respire-agent-locks-redis` container in particular is shared lock infrastructure — never remove it).
...
Respire tracks decomposition with GitHub **native sub-issues** (parent/child) and **dependencies** (`blocked_by` / `blocking`) when available, not markdown checklists as source of truth. Prefer native `blocked_by` relationships for blockage. Repo is `thomhurst/Respire`. Write calls need the numeric issue `.id` (an integer distinct from `#number`, passed with `-F`, not `-f`).
...
```powershell
gh api repos/thomhurst/Respire/issues/<N>/sub_issues --paginate --jq '.[] | "\(.number) [\(.state)] \(.title)"'
gh api repos/thomhurst/Respire/issues/<N>/dependencies/blocked_by --jq '[.[].number]'
```
...
3. Link the child to the parent as a native sub-issue:
   ...
   ```powershell
   gh api --method POST repos/thomhurst/Respire/issues/<N>/sub_issues -F sub_issue_id=$childId
   ```

4. For ordering, mark a child blocked by its prerequisite (use the prerequisite's `.id`):

   ```powershell
   gh api --method POST repos/thomhurst/Respire/issues/<blockedNum>/dependencies/blocked_by -F issue_id=<blockingId>
   ```
...
```powershell
rg "<symbol|type|module|options class|extension>" src tests docs benchmarks samples website -g "*.cs" -g "*.md" -g "*.json" -g "*.yml"
```
...
4. Run focused tests, broader tests, and `npm run build --prefix website` for website/docs changes. Every local `dotnet` command must run through `pwsh scripts/Invoke-AgentDotNet.ps1` with the shell/tool timeout at least 30 seconds longer than the guard timeout; never invoke `dotnet` directly. Tests are TUnit on Microsoft.Testing.Platform: run the relevant project under `tests/` with guarded arguments equivalent to `dotnet run --project tests/<TestProject> --framework net10.0`, and use `--treenode-filter "/Assembly/Namespace/Class/Test"` (wildcards allowed) for focus — never VSTest `--filter`. Use one `--treenode-filter` pattern per command unless using valid TUnit OR parentheses. Prefer `Respire.Tests` / `Respire.Pipeline.Tests` for fast feedback; `Respire.IntegrationTests` needs Docker (Testcontainers Redis). Guard exit 124 (timeout) or 137 (memory) is a local validation limit: do not raise the limit and retry automatically; report it in the PR and let CI run the expensive check.
...
- **Generated command catalog is off-limits:** `src/Respire/RespireCommands.g.cs` is auto-generated by `tools/Generate-CommandCatalog.ps1` and will be overwritten. Never hand-edit it; change the generator or add a manual partial/extension file instead.
...
- The networking core (`src/Respire/Networking`) has strict wire-layer invariants: FIFO command/response ordering, refcounted buffers, in-flight sends are never cancelled mid-write, and a persistent flush task. Do not "simplify" these away; understand the invariant before touching that layer.
- Never run the full benchmark suite (`run-benchmarks.ps1` / `benchmarks/`) as an agent. BenchmarkDotNet runs are long and memory-hungry; the benchmark CI workflows own performance validation. If Docker is unavailable for focused tests that need it, state that clearly in the PR test plan.
