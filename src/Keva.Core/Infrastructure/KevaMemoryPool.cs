...
/// <summary>
/// High-performance memory pool for Keva operations
/// Provides pooled memory management to reduce GC pressure
/// </summary>
public sealed class KevaMemoryPool : IDisposable
{
    ...
    public static readonly KevaMemoryPool Shared = new();
    
    private KevaMemoryPool()
    {
    ...
    /// <summary>
    /// Allocates a pinned high-performance buffer for zero-copy operations (inspired by KevaDirectClient)
    ...
    {
        if (_disposed) throw new ObjectDisposedException(nameof(KevaMemoryPool));
    }
...
{
    private readonly KevaMemoryPool _pool;
    private byte[] _buffer;
    private int _position;
    private bool _disposed;
    
    internal PooledBufferWriter(KevaMemoryPool pool, int initialCapacity)
    {
        _pool = pool;
