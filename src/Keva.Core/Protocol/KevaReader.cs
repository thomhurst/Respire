...
/// </summary>
public ref struct KevaReader
{
    private ReadOnlySpan<byte> _buffer;
    private int _position;
    
    public KevaReader(ReadOnlySpan<byte> buffer)
    {
        _buffer = buffer;
        _position = 0;
    }
    ...
    public bool TryRead(out KevaValue value)
    {
        value = default;
    ...
    private bool TryReadSimpleString(out KevaValue value)
    {
        ...
        value = KevaValue.SimpleString(bufferRef, start, length);
        return true;
    }
    ...
    private bool TryReadError(out KevaValue value)
    {
        ...
        var bufferRef = new ReadOnlyMemory<byte>(_buffer.ToArray());
        value = KevaValue.Error(bufferRef, start, length);
        return true;
    }
    ...
    private bool TryReadInteger(out KevaValue value)
    {
        ...
        value = KevaValue.Integer(longValue);
        return true;
    }
    ...
    private bool TryReadBulkString(out KevaValue value)
    {
        ...
        var bufferRef = new ReadOnlyMemory<byte>(_buffer.ToArray());
        value = KevaValue.BulkString(bufferRef, dataStart, (int)length);
        return true;
    }
    ...
    private bool TryReadBoolean(out KevaValue value)
    {
        ...
        value = boolChar == 't' ? KevaValue.True : KevaValue.False;
        return true;
    }
