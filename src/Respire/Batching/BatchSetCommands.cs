...
internal sealed class BatchSetCommands(IPendingSink sink) : IBatchSetCommands
{
    ...
    public RespirePending<string[]> Members(RespireKey key)
    ...
    public RespirePending<long> IntersectStore(RespireKey destination, params ReadOnlySpan<RespireKey> keys)
        => StoreKeys("SINTERSTORE", Verbs.SInterStore, destination, keys);

    public RespirePending<long> UnionStore(RespireKey destination, params ReadOnlySpan<RespireKey> keys)
        => StoreKeys("SUNIONSTORE", Verbs.SUnionStore, destination, keys);

    public RespirePending<long> DifferenceStore(RespireKey destination, params ReadOnlySpan<RespireKey> keys)
